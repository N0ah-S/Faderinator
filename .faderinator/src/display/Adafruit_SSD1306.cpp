...
#define PROGMEM /*arduino specific*/
...
/*!
    ...
    @param  twi
            Pointer to an existing i2c_rp2040 instance (e.g. &Wire, the
...
*/
Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, i2c_rp2040_wire* twi,
                                   int8_t rst_pin, uint32_t clkDuring,
                                   uint32_t clkAfter)
    : Adafruit_GFX(w, h), wire(twi), buffer(NULL), rstPin(rst_pin) /*wireClk(clkDuring), restoreClk(clkAfter)*/
{
}
...
/*!
    ...
    @param  periphBegin
            ...
            (e.g. a i2c_rp2040 interface on non-default pins, as can be done
...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset,
  ...
  /*if (reset && (rstPin >= 0)) {
    ...
    TRANSACTION_START// i2c_rp2040 begin() function might be already performed by the calling
    i2caddr = addr ? addr : ((HEIGHT == 32) ? 0x3C
                                            : 0x3D);// function if it has unusual circumstances (e.g. TWI variants that
